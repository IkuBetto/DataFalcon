...
do
  EVENT="{\"date\":\"${DATE}\"}"
  (echo "[`date`] Start executing ${EXEC_NAME}"; python -c "import ${EXEC_NAME}; ${EXEC_NAME}.lambda_handler(${EVENT}, 'context')"; echo "[`date`] Finish executing ${EXEC_NAME}") 2>&1 >${LOG_DIR}/${EXEC_NAME}_${DATE}.log &
done
