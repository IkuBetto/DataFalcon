    ...
    for all_past_race_result_index in all_past_race_result_df.index:
        ...
        if past_horse_rank.isdecimal():
            try:
                ...
                print("%s %s を飛ばします" % (past_race_name, past_horse_rank))
            
            # 次走平均順位の計算
            if upper_df.shape[0] > 0:
                uppper_next_average_rank = upper_df[upper_df["next_average_rank"] != 0].mean()["next_average_rank"]
            else:
                uppper_next_average_rank = "-"
            ...
            upper_rank_list.append(uppper_next_average_rank)
